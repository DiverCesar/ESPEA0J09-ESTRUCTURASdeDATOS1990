...
#include <stdlib.h>
using namespace std;
const int N = 3; //Se declara una constante de tipo entero (Hay que recordar que estos se necesitaran para la declaracion del arreglo)
const int M = 3;
float matriz[N] [M]; //Se declara un arreglo de dos dimensiones
int fila, col, ran; //Se declaran dos variables las cuales serán usadas para localizar las filas y las columnas
...
void leerarray()
{
 ...
 {
 ran = 1+rand()%9;
 cout << "Valor para Fila: " << fila+1 << " - Columna: " << col+1 << " = "<<ran; //Se pide al usuario que ingrese el valor para la fila y columna indicada
 matriz[fila][col] = ran; //Se realiza la lectura desde teclado (nótese que en los corchetes, al igual que en los ciclos for, primero se pone la fila y después la columna)
